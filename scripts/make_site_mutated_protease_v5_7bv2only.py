...
from pyrosetta.rosetta.core.select.residue_selector import AndResidueSelector, \
    ...
    TrueResidueSelector, NeighborhoodResidueSelector
...
def parse_args():
    ...
    parser.add_argument('-m', '--mutants_list', type=str, required=True, 
        help='Input a fasta list file identifying the mutations.')
    ...
    parser.add_argument('-fix_rna', '--not_repacking_rna', action='store_true', 
        help='Giving a flag of -fix_rna will prevent the RNA motif from repacking.')
...
def compare_sequences(pdb_file, seq_2, query_seq, ind_by): #ind_by has two option, pose or pdb
    """
    ...
    wild_pose = pr.pose_from_pdb(pdb_file)
        ...
        if align_ind_1 < align_ind_2:
            for more_ind in range(0, align_ind_2 - align_ind_1):
                if query_seq[more_ind] != seq_2[more_ind]: 
                    site_changes.append((start_ind - (align_ind_2 - align_ind_1 ) + more_ind, query_seq[more_ind], seq_2[more_ind], False)) # label that additions on N terminal for reference sequences; add one more column to judge if it is in pdb or not
        ...
        if len(new_seq_1.replace("-","")) != len(wild_seq): # need to be modified generally, len(new_seq_1) > len(new_seq_2)
            ...
            for co_C in range(start_ind_in_seq2, len(seq_2)):
                if query_seq[co_C] != seq_2[co_C]:
                    site_changes.append((start_ind + co_C - (align_ind_2 - align_ind_1) + len(find(new_seq_2, "-")), query_seq[co_C], seq_2[co_C], False)) # just to symbol len(ref) > len(wild)
    ...
    for n, i in enumerate(new_seq_2):
        if i != query_seq_aligned[n]:
            ...
            seq_2_value = i
            site_changes.append((site, seq_1_value, seq_2_value, new_seq_1[n] != "-"))
...
def make_point_mutant_task_factory_wo_repacking_rna(site_changes, ex12=True, \
    ...
    rna_selection = ChainSelector('P,T')
    ...
    if repacking_range:
        ...
        res_wo_rna_selection = NotResidueSelector(rna_selection)
        repacking_res_wo_rna_selection = AndResidueSelector(\
            repacking_res_selection, res_wo_rna_selection)
        
        repack = RestrictToRepackingRLT()
        tf.push_back(OperateOnResidueSubset(repack, repacking_res_wo_rna_selection))

        mutated_and_repacking_res_selection = OrResidueSelector(\
            mutated_res_selection, repacking_res_wo_rna_selection)
        prevent = PreventRepackingRLT()
        tf.push_back(OperateOnResidueSubset(prevent, mutated_and_repacking_res_selection, \
            True))

    else:
        mutated_res_and_rna_selection = OrResidueSelector(mutated_res_selection, \
            rna_selection)
        repack = RestrictToRepackingRLT()
        tf.push_back(OperateOnResidueSubset(repack, mutated_res_and_rna_selection, True))

        prevent = PreventRepackingRLT()
        tf.push_back(OperateOnResidueSubset(prevent, rna_selection))
...
def make_mutant_model(ref_pose, substitutions, score_function, ex12=True, \
    repacking_range=False, relax_decoys=False, not_repacking_rna=False, \
    ...
    if len(substitutions) > 0:
        if not_repacking_rna:
            tf = make_point_mutant_task_factory_wo_repacking_rna(substitutions, \
...
def analyze_mutant_protein(seqrecord, ref_pose, sf, query, pdb_name, ind_type, main_chain=1,  
    make_model=True, ex12=True, oligo_chains=None, cat_res=None, substrate_chains=None,
    repacking_range=False, relax_decoys=False, not_repacking_rna=False, debugging_mode=False):
    """
    ...
    ref_seq = ref_pose.chain_sequence(main_chain)
    ...
    if make_model:
        mutated_pose, mut_pose_info = \
            make_mutant_model(ref_pose, new_subs, sf,  ex12=ex12, \
                ...
                not_repacking_rna=not_repacking_rna, debugging_mode=debugging_mode)
    ...
    return mut_df, mutated_pose, substitutions, new_subs
...
def main(args):
    ...
    fasta_list = parse_fastafile(args.mutants_list)
    ...
    all_substitutions = []
    for n, mutant in enumerate(analyze_list):
        single_mutant_info, mutated_pose, substitutions, new_subs = \
            analyze_mutant_protein(mutant, wild_pose, sf, wt, 
                ...
                not_repacking_rna=args.not_repacking_rna)
