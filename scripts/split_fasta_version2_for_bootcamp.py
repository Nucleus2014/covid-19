...
if args.check == True:
	...
	for s in refs:
		...
		hyphen_list = find(que_out, "-") # return all indices of '-'
		ref_out = tmpAlign.to_string().strip().split("\n")[2].strip().split()

		new_ref = ''.join([ref_out[2][sel] for sel in range(len(ref_out[2])) if sel not in hyphen_list])
		refs_out.append(new_ref) # save reference sequence
...
for ind in range(n_splits):
	...
	gp.write(query_name + "\n")
	seq_split_batch = math.floor(len(wild_seq) / 60)
	for batch in range(seq_split_batch):
		tmp = wild_seq[batch * 60 : (batch + 1) * 60]
		gp.write(str(tmp) + "\n")
	gp.write(wild_seq[(batch + 1) * 60 :] + "\n")	
	...
	if ind < n_splits -1:
		...
		for q in range(0,len(remain_keys)):
			...
			for batch in range(seq_split_batch):
				gp.write(refs_out[q][batch * 60 : (batch + 1) * 60])
				gp.write("\n")
