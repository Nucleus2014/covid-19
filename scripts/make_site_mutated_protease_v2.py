...
def parse_args():
    ...
    parser.add_argument('-debug', '--debugging_mode', action='store_true', 
        help='Giving a flag of -debug will let the task factory show which \
        residues are selected to make point mutations, repack or stay static.')
...
def fast_relax_pose_with_muts(pose, task_factory, score_function, decoys):
    ...
    job_distributor = pr.PyJobDistributor(pose.pdb_info().name()[:-4], decoys, score_function)
    if_first_decoy = True
    lowest_energy = 10000
    while not job_distributor.job_complete:
        ...
        if decoy_energy < lowest_energy or if_first_decoy:
...
def analyze_mutant_protein(seqrecord, ref_pose, sf, main_chain=1, make_model=True, \
    ex12=True, oligo_chains=None, cat_res=None, substrate_chains=None, \
        repacking_range=False, relax_decoys=False, debugging_mode=False):
    """
    ...
    substitutions = compare_sequences(ref_seq, seqrecord.seq)
    ...
    mut_tags['multiple'] = len(substitutions) > 1
    ...
    if make_model:
        mutated_pose, mut_pose_info = \
            make_mutant_model(ref_pose, substitutions, sf, ex12=ex12, \
    ...
    mut_tags['coevolution'] = check_coevolution(pose, sf, substitutions)
    ...
    if oligo_chains:
        interface_checks = [check_interface_proximity(pose, sf, pm[0], 
            sym_chains, main_chain=main_chain) for pm in substitutions]
    ...
    if substrate_chains:
        substrate_checks = [check_interface_proximity(pose, sf, pm[0], 
            substrate_chains, main_chain=main_chain) for pm in substitutions]
        mut_tags['near_substrate'] = ';'.join([str(i) for i in substrate_checks])

    # Convert to a DataFrame
    mut_df = pd.DataFrame(mut_tags, index=[1])

    return mut_df, mutated_pose, substitutions
...
def main(args):
    ...
    for n, mutant in enumerate(analyze_list):
        single_mutant_info, mutated_pose, substitutions = \
