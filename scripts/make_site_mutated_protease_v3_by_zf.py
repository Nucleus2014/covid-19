...
def compare_sequences(pdb_file, seq_2, ind_by): #ind_by has two option, pose or pdb
    """
    ...
    if align_ind_1 != 1 or align_ind_2 != 1:
        if align_ind_1 < align_ind_2:
            site_changes.append((start_ind - 1, "-", "-")) # label that additions on N terminal for reference sequences
            ...
            for ca_N in range(0, align_ind_1 - align_ind_2):
                site_changes.append((str(start_ind + ca_N), wild_seq[ca_N], "-"))
    # # C-terminal truncations has already been truncated in aligned seq_1, record C-terminal truncations
    if new_seq_2[-1] == "-":
        if len(new_seq_1.replace("-","")) != len(wild_seq): # need to be modified generally
            ...
            for co_C in range(len_in_wild, len(wild_seq)):
                site_changes.append((str(start_ind + len_before_1 - len_in_wild + co_C), wild_seq[co_C], "-"))
        else:
            new_seq_2 = new_seq_2.strip("-")
            site_changes.append((start_ind + len(new_seq_2), "-", "-")) # just to symbol len(ref) > len(wild)
...
def make_mutant_model(ref_pose, substitutions, score_function, ex12=True, \
    repacking_range=False, relax_decoys=False, debugging_mode=False):
    """
    ...
    Modified by Zhuofan.
    """
    # First generate a new reference pose by repacking all residues and minimization
    tf = make_point_mutant_task_factory(substitutions, ex12=ex12)
    repacked_minimized_ref_pose = make_point_changes(ref_pose, tf, score_function)

    # If making point matations
    if len(substitutions) > 0:
        # Repack the residues surrounding point mutations / repack all residues + minimize
        tf = make_point_mutant_task_factory(substitutions, ex12=ex12, \
    ...
    # If there is no point mutation in the input sequence
    elif len(substitutions) == 0:
        mutated_pose = repacked_minimized_ref_pose
    
    # Print out the task operations on all residues 
    if debugging_mode:
    ...
    # Get 'wt_energy' from scoring 'repacked_minimized_ref_pose' instead of 'ref_pose'
    wt_energy = total_energy(repacked_minimized_ref_pose, score_function)
...
def analyze_mutant_protein(seqrecord, ref_pose, sf, ind_type, pdb_name, main_chain=1,  
    ...
    substitutions, new_subs = compare_sequences(pdb_name, seqrecord.seq, ind_type)
    ...
    conservations = [is_conservative(pm) for pm in new_subs]
...
def main(args):
    ...
    for n, mutant in enumerate(analyze_list):
        single_mutant_info, mutated_pose, substitutions, new_subs = \
            analyze_mutant_protein(mutant, wild_pose, sf, 
        ...
        if args.make_models:
            ...
            mut_summary = [get_mutant_brief(s) for s in subs_clean]
