...
def compare_sequences(pdb_file, seq_2, query_seq, ind_by): #ind_by has two option, pose or pdb
        ...
        if align_ind_1 < align_ind_2:
            for more_ind in range(0, align_ind_2 - align_ind_1):
                if query_seq[more_ind] != seq_2[more_ind]: 
                    site_changes.append((0, query_seq[more_ind], seq_2[more_ind], False)) # label that additions on N terminal for reference sequences; add one more column to judge if it is in pdb or not
    ...
    if new_seq_2[-1] == "-":
        if len(new_seq_1.replace("-","")) != len(wild_seq): # need to be modified generally, len(new_seq_1) > len(new_seq_2)
            ...
            if align_ind_1 < align_ind_2:
                start_ind_in_seq2 = len(new_seq_2) + align_ind_2 - 1
            ...
            for co_C in range(start_ind_in_seq2, len(seq_2)):
                if query_seq[co_C] != seq_2[co_C]:
                    site_changes.append((999, query_seq[co_C], seq_2[co_C], False)) # just to symbol len(ref) > len(wild)
...
def main(args):
    ...
    all_subs_info.columns = ['site', 'native', 'mutant','is_in_PDB']
